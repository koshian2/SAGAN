...
def train(cases):
    ...
    dataloader = load_flower(batch_size)

    model_G = resnet96.Generator(n_classes_g=102)
    model_D = resnet96.Discriminator(n_classes_d=102)
    ...
    for epoch in range(n_epoch):
        ...
        for i, (real_img, labels) in tqdm(enumerate(dataloader), total=n):
            ...
            fake_onehots = onehot_encoding[torch.randint(0, 102, (batch_size,))].to(device)            
            ...
            # train fake
            rand_X = torch.randn(batch_len, 128).to(device)
            fake_onehots = onehot_encoding[torch.randint(0, 102, (batch_len,))].to(device)
